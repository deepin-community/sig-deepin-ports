...
- `deepin-k3-riscv64-25-desktop-installer.sha256sum`
  sha256 校验和
- `deepin-k3-riscv64-25-desktop-installer.md5sum`
  md5 校验和
- `deepin-k3-riscv64-25-desktop-installer.boot.ext4`
  系统启动目录分区镜像
- `deepin-k3-riscv64-25-desktop-installer.root.ext4`
  系统根目录分区镜像

校验镜像：

``` shell
$ cat deepin-k3-riscv64-25-desktop-installer.sha256sum
...
前往[deepin-riscv-kernel](https://github.com/deepin-community/deepin-riscv-kernel/actions)，进入最新的 `build all` 项，在 Artifacts 中选择 `uboot-k3-spacemit` 下载并解压。

从仓库中获取 esos deb 文件并 dpkg-deb -x 获得 esos.itb
...
> 如果分区已存在，可以不用重复创建，直接跳过此步即可。

bootloader 单独写在 spi 存储中。
...
# 刷写完 mtd 后进入 mtd 刷写模式，可以执行「写入 bootloader」操作
fastboot flash gpt partitions_scsi.json
# 刷写完 gpt 后进入系统刷写模式，可以执行「刷写操作系统」操作
```
...
```bash
fastboot flash bootfs deepin-k3-riscv64-25-desktop-installer.boot.ext4
